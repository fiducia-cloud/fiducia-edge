...
try {
  ...
  const health = await visitScenario("health");
  assert.equal(health.status, 200);
  assert.match(health.bodyText, /fiducia-edge/);
  assertHardenedHeaders(health.headers);

  const missing = await visitScenario("missing-credentials");
  ...
  const invalid = await visitScenario("invalid-jwt", {
    Authorization: `Bearer ${invalidToken}`,
  });
  assert.equal(invalid.status, 401);
  assert.match(invalid.bodyText, /invalid_jwt/);
  assert.doesNotMatch(invalid.bodyText, new RegExp(invalidToken.replaceAll(".", "\\.")));
  assertNoStore(invalid.headers);
  assertHardenedHeaders(invalid.headers);

  const misconfigured = await visitScenario("misconfigured-policy");
  ...
  const upstreamFailure = await visitScenario("upstream-failure");
  ...
  const csp = await visitScenario("csp-enforcement");
...
async function visitScenario(name, extraHeaders = {}) {
  const url = `${fixture.origin}/scenario/${name}`;
  ...
  const scriptRan = await evaluate("globalThis.__fiduciaScriptRan === true");
  const result = {
    name,
...
async function startFixtureServer() {
  const server = createServer(async (request, response) => {
    ...
    try {
      ...
      response.end(JSON.stringify({ error: "fixture_failure", type: error?.name || "Error" }));
    }
  });
    ...
    case "csp-enforcement":
      return {
        ...
        fetchImpl: async () => new Response(
          "<!doctype html><main id=probe>browser-policy-probe</main>" +
...
async function startChrome() {
  ...
  if (version.status !== 0) throw new Error(`unable to execute browser: ${executable}`);
  ...
  while (Date.now() < deadline) {
    ...
    try {
      ...
      // Chrome creates the file after its DevTools endpoint is ready.
    }
    await delay(50);
  }
...
async function connectCdp(debugPort) {
  ...
  if (!response.ok) throw new Error(`DevTools target creation failed: ${response.status}`);
...
class CdpClient {
  constructor(socket) {
    ...
    socket.addEventListener("close", () => this.failPending(new Error("DevTools websocket closed")));
  }
...
async function captureFailureScreenshot() {
  ...
  try {
    ...
    // The report still captures the primary failure if the browser has exited.
  }
}
